...
function Optimizer({ returnData, portfolios, onApplyWeightsToPortfolio }) {
  ...
  const [applyTargetPortfolio, setApplyTargetPortfolio] = useState(null);
  ...
  const addAllAssets = () => {
    setIncluded(new Set(ASSETS.map((a) => a.id)));
  };
  ...
  const handleApply = (weights, assets, targetPortfolioId) => {
    if (!onApplyWeightsToPortfolio || !weights || !assets) return;
    const portfolioAssets = assets.map((a, i) => ({
      ...a,
      weight: Math.round(weights[i] * 1000) / 10,
    })).filter((a) => a.weight > 0.1);

    const targetId = targetPortfolioId || applyTargetPortfolio || portfolios?.[0]?.id;
    if (targetId) {
      onApplyWeightsToPortfolio(portfolioAssets, targetId);
    }
  };
    ...
    <div className="fade-in">
      <div className="tab-header">
        <div>
          ...
          <div className="tab-description">Find optimal weights for your selected assets</div>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '300px 1fr', gap: 'var(--space-lg)' }}>
        ...
        <div>
          <div className="config-block" style={{ marginBottom: 'var(--space-md)' }}>
            <div className="config-block-title" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span>Include Assets ({included.size})</span>
              <div style={{ display: 'flex', gap: 4 }}>
                <button className="btn btn-secondary btn-sm" onClick={addAllAssets}>All</button>
                <button className="btn btn-secondary btn-sm" onClick={clearAllAssets}>None</button>
              </div>
            </div>
                  ...
                  <div key={category} style={{ marginBottom: 4 }}>
                    <div style={{
                      ...
                      <input type="checkbox" checked={allChecked}
                        ...
                        onChange={() => toggleCategory(category)} style={{ cursor: 'pointer' }} />
                    ...
                    {isExpanded && catAssets.map((asset) => (
                      <label key={asset.id} style={{
                        ...
                        padding: '3px 8px 3px 24px', cursor: 'pointer', fontSize: '0.8125rem',
                        color: included.has(asset.id) ? 'var(--text-primary)' : 'var(--text-muted)',
                      }}>
                        <input type="checkbox" checked={included.has(asset.id)}
                          onChange={() => toggleAsset(asset.id)} style={{ cursor: 'pointer' }} />
          ...
          <div className="config-block">
            ...
            <div className="config-row">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <input type="number" min="0" max="50" step="1" value={minWeight}
            ...
            <div className="config-row">
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                <input type="number" min="5" max="100" step="5" value={maxWeight}
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-md)', marginBottom: 'var(--space-md)' }}>
            ...
            <div className="toggle-group">
              <button className={`toggle-option ${mode === 'simulated' ? 'active' : ''}`} onClick={() => setMode('simulated')}>Simulated</button>
              <button className={`toggle-option ${mode === 'actual' ? 'active' : ''}`} onClick={() => setMode('actual')}>Actual</button>
            </div>
            {mode === 'actual' && Object.keys(returnData).length === 0 && (
              <span className="warning-text">Historical data loading...</span>
            )}
          </div>
          ...
          <button className={`run-button ${isRunning ? 'running' : ''}`}
            onClick={handleOptimize} disabled={isRunning || includedAssets.length < 2}
          ...
          {results?.error && (
            <div className="card" style={{ color: 'var(--accent-danger)', marginBottom: 'var(--space-md)' }}>{results.error}</div>
          )}

          {results && objective === 'pareto' && results.frontier && (
            <ParetoChart results={results} selectedIdx={selectedFrontierIdx} onSelectPoint={setSelectedFrontierIdx} />
          )}
            ...
            <div className="chart-container">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-md)', flexWrap: 'wrap', gap: 'var(--space-sm)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-sm)' }}>
                  <span style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>Apply to:</span>
                  <select value={applyTargetPortfolio || availablePortfolios[0]?.id || ''}
                    onChange={(e) => setApplyTargetPortfolio(parseInt(e.target.value))}
                    style={{ fontSize: '0.8125rem', padding: '4px 8px' }}>
                    {availablePortfolios.map((p) => (
                      <option key={p.id} value={p.id}>
                        {p.name} {p.assets.length === 0 ? '(empty)' : `(${p.assets.length} assets)`}
                      </option>
                    ))}
                  </select>
                  <button className="btn btn-primary btn-sm"
                    onClick={() => handleApply(displayWeights, results.assets || includedAssets, applyTargetPortfolio || availablePortfolios[0]?.id)}>
                    Apply
                  </button>
                </div>
              </div>
              <WeightChart weights={displayWeights} assets={results.assets || includedAssets} />
            </div>
          )}

          {displayMetrics && !results?.error && <MetricsDisplay metrics={displayMetrics} />}
          ...
          {!results && includedAssets.length >= 2 && (
            <div className="chart-container">
              ...
              <table>
                <thead><tr><th>Asset</th><th>Expected Return</th><th>Volatility</th><th>Type</th></tr></thead>
                <tbody>
...
function WeightChart({ weights, assets }) {
  const data = assets
    .map((a, i) => ({ ticker: a.ticker || a.id, name: a.name, weight: Math.round(weights[i] * 1000) / 10 }))
    ...
    <div>
      <ResponsiveContainer width="100%" height={Math.max(200, data.length * 32 + 40)}>
        <BarChart data={data} layout="vertical" margin={{ top: 5, right: 30, left: 60, bottom: 5 }}>
          ...
          <XAxis type="number" tickFormatter={(v) => `${v}%`} fontSize={11} tick={{ fill: '#65676b' }} />
          <YAxis type="category" dataKey="ticker" fontSize={11}
            tick={{ fill: '#0f1419', fontFamily: 'var(--font-mono)', fontWeight: 600 }} width={55} />
          <Tooltip formatter={(value, name, { payload }) => [`${value.toFixed(1)}%`, payload.name]}
      ...
      <table style={{ marginTop: 'var(--space-sm)' }}>
        ...
        <tbody>
          {data.map((d) => (<tr key={d.ticker}><td style={{ fontWeight: 600 }}>{d.ticker}</td><td>{d.weight.toFixed(1)}%</td></tr>))}
        </tbody>
      </table>
    </div>
  );
...
function MetricsDisplay({ metrics }) {
  ...
  return (
    <div className="stats-grid" style={{ marginTop: 'var(--space-md)' }}>
      ...
      <div className="stat-card"><div className="stat-value">{fmt(m.vol)}%</div><div className="stat-label">Volatility</div></div>
      <div className="stat-card"><div className="stat-value">{fmt(m.sharpe, 2)}</div><div className="stat-label">Sharpe Ratio</div></div>
      <div className="stat-card"><div className="stat-value">{fmt(m.sortino, 2)}</div><div className="stat-label">Sortino Ratio</div></div>
    </div>
  );
}

function ParetoChart({ results, selectedIdx, onSelectPoint }) {
  const { frontier, cloud, maxSharpe, minVol, riskParity } = results;
  const frontierData = frontier.map((p, i) => ({ vol: p.vol, cagr: p.cagr, sharpe: p.sharpe, idx: i }));
  const renderDot = (props) => {
    const { cx, cy, payload } = props;
    const isSelected = payload.idx === selectedIdx;
    return (
      <circle cx={cx} cy={cy} r={isSelected ? 6 : 3}
        fill={isSelected ? '#1d9bf0' : '#0f1419'}
        stroke={isSelected ? '#0f1419' : 'none'} strokeWidth={isSelected ? 2 : 0}
        style={{ cursor: 'pointer' }} onClick={() => onSelectPoint(payload.idx)} />
    );
  };
  ...
  return (
    <div className="chart-container" style={{ marginBottom: 'var(--space-md)' }}>
      ...
      <div className="chart-legend">
        <div className="legend-item"><div className="legend-dot" style={{ background: 'rgba(0,0,0,0.08)' }} /> All Portfolios</div>
        <div className="legend-item"><div className="legend-dot" style={{ background: '#0f1419' }} /> Frontier</div>
        <div className="legend-item"><div className="legend-dot" style={{ background: '#1d9bf0', width: 10, height: 10 }} /> Max Sharpe</div>
        <div className="legend-item"><div className="legend-dot" style={{ background: '#00ba7c', width: 10, height: 10 }} /> Min Vol</div>
        <div className="legend-item"><div className="legend-dot" style={{ background: '#f59e0b', width: 10, height: 10 }} /> Risk Parity</div>
      </div>
      <ResponsiveContainer width="100%" height={400}>
        <ScatterChart margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="rgba(0,0,0,0.06)" />
          <XAxis dataKey="vol" name="Volatility" tickFormatter={(v) => `${v.toFixed(0)}%`} fontSize={11} tick={{ fill: '#65676b' }}
            label={{ value: 'Volatility (%)', position: 'bottom', offset: -5, fontSize: 11, fill: '#65676b' }} />
          <YAxis dataKey="cagr" name="CAGR" tickFormatter={(v) => `${v.toFixed(0)}%`} fontSize={11} tick={{ fill: '#65676b' }}
          ...
          <Scatter data={cloud} fill="rgba(0,0,0,0.06)" shape="circle" legendType="none" isAnimationActive={false}>
            {cloud.map((_, i) => (<Cell key={i} r={1.5} />))}
          </Scatter>
          <Scatter data={frontierData} fill="#0f1419" shape={renderDot} line={{ stroke: '#0f1419', strokeWidth: 1.5 }} legendType="none" isAnimationActive={false} />
          {maxSharpe && <ReferenceDot x={maxSharpe.vol} y={maxSharpe.cagr} r={8} fill="#1d9bf0" stroke="#0f1419" strokeWidth={2} label={{ value: 'Sharpe', fontSize: 10, position: 'top' }} />}
          {minVol && <ReferenceDot x={minVol.vol} y={minVol.cagr} r={7} fill="#00ba7c" stroke="#0f1419" strokeWidth={2} label={{ value: 'Min Vol', fontSize: 10, position: 'left' }} />}
          {riskParity && <ReferenceDot x={riskParity.vol} y={riskParity.cagr} r={7} fill="#f59e0b" stroke="#0f1419" strokeWidth={2} label={{ value: 'RP', fontSize: 10, position: 'right' }} />}
        </ScatterChart>
      </ResponsiveContainer>
